...
import { workspace, Disposable, WorkspaceConfiguration } from 'coc.nvim'
...
export class TreeViewsManager implements Disposable {
  ...
  public async prepareWindowForGoto(): Promise<any> {
    const windowProp = Date.now() - this.lastWindowProp[1] < 1000 ? this.lastWindowProp[0] : WindowProp.Default
    if (windowProp === WindowProp.Tab) {
      ...
      if (mbWindow === undefined) {
        ...
        if (windows.length > 0) {
          ...
          await this.nvim.command(`silent vertical resize ${fullWidth - initWidth}`)
        } else {
          await this.nvim.command(`silent ${position} vertical new`)
        }
      } else {
