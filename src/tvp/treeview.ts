...
import { commands, workspace, Disposable, WorkspaceConfiguration } from 'coc.nvim'
...
export class TreeView implements Disposable {
  ...
  public async init(): Promise<void> {
    ...
    function cocCommand(param: string): string { return `:<C-u>CocCommand metals.tvp.view ${param} <CR>` }
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("toggleNode")} ${cocCommand("ToggleNode")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("forceChildrenReload")} ${cocCommand("ForceChildrenReload")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("gotoParentNode")} ${cocCommand("ParentNode")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("gotoFirstChild")} ${cocCommand("FirstSibling")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("gotoLastChild")} ${cocCommand("LastSibling")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("gotoPrevSibling")} ${cocCommand("PrevSibling")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("gotoLastSibling")} ${cocCommand("NextSibling")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("executeCommand")} ${cocCommand("ExecuteCommand")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("executeCommandAndOpenSplit")} ${cocCommand("ExecuteCommandAndOpenSplit")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("executeCommandAndOpenVSplit")} ${cocCommand("ExecuteCommandAndOpenVSplit")}`)
    await this.nvim.command(`${commandPrefix} ${this.config.get<string>("executeCommandAndOpenTab")} ${cocCommand("ExecuteCommandAndOpenTab")}`)
  ...
  public handleModelUpdates(ev0: TreeModelUpdate): void {
    ...
    this.eventQueue = this.eventQueue.then(async _ => {
      ...
      if (ev.focusEvent) {
        this.logger.debug(`Focus event. Root: ${ev.root.underlying.nodeUri}, offset: ${offset}.`)
        ...
        this.logger.debug(`Update event. Root: ${ev.root.underlying.nodeUri}, length: ${ev.oldNodes.length}, payload: ${payload}.`)
        await this.modifyBuffer(async () => {
          ...
          if (this.firstUpdate) {
            ...
            await this.buffer.remove(ev.newNodes.length, ev.newNodes.length + 1)
          }
        })
      }
    })
  ...
  private async insertRows(offset: number, nodes: NodeView[]): Promise<void> {
    ...
    await Promise.all(nodes.map((node, idx) => {
      ...
      if (highlightSchema !== undefined) {
        ...
        this.logger.debug(`signplace id=${placeId} schema=${highlightSchema} num=${line} uri=${node.underlying.nodeUri}`)
